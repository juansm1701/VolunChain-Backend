...
import {
  TransactionHelper,
  withTransaction,
} from "../../src/utils/transaction.helper";
...
describe("TransactionHelper", () => {
  ...
  describe("executeInTransaction", () => {
    it("should execute callback within transaction successfully", async () => {
      ...
      (mockPrismaClient.$transaction as jest.Mock).mockImplementation(
        (callback) => callback(mockTransaction)
      );
    ...
    it("should handle transaction failure", async () => {
      ...
      await expect(
        transactionHelper.executeInTransaction(mockCallback)
  ...
  describe("executeParallelInTransaction", () => {
    it("should execute multiple operations in parallel", async () => {
      ...
      const results =
        await transactionHelper.executeParallelInTransaction(operations);
  ...
  describe("executeSequentialInTransaction", () => {
    it("should execute operations in sequence", async () => {
      ...
      const results =
        await transactionHelper.executeSequentialInTransaction(operations);
