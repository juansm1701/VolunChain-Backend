import { ResendVerificationEmailUseCase } from '../../src/modules/auth/use-cases/resend-verification-email.usecase';
...
describe('ResendVerificationEmailUseCase', () => {
  ...
  beforeAll(async () => {
    ...
    useCase = new ResendVerificationEmailUseCase(userRepository);
  });
  ...
  test('should resend verification email to unverified user', async () => {
    ...
    const testUser: IUser = {
      ...
      verificationTokenExpires: new Date(Date.now() - 1000) // expired token
    };
    
    await userRepository.create(testUser);
    
    // Save original and set environment variable for the test
    ...
    // Check that setVerificationToken was called with a new token
  ...
  test('should not resend email if user is already verified', async () => {
